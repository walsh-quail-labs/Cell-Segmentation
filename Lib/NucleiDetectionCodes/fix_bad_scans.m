...
%         plot( rotated_ellipse(1,:)/4,rotated_ellipse(2,:)/4,'-');
        if ~isempty(rotated_ellipse)
            bx = min(max(round(rotated_ellipse(1,:)/4),1),size(nucleiImage,2));
            by = min(max(round(rotated_ellipse(2,:)/4),1),size(nucleiImage,1));
            bx = bx';
            by = by';
...
save(nucleiImagePath,'nucleiOccupancyIndexed', 'Boundaries', 'nucleiImage', 'occupancy_image');
