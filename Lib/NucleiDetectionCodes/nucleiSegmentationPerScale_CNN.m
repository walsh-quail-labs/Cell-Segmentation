function [allSegBoundaries,finalMask]=nucleiSegmentationPerScale_CNN(result,WindowSize,occpuiedAreaThresh,double_scale,scale,resizeScale,diskScale,tempPath)
...
for stX = 1:WindowSize:endItrX
    for stY = 1:WindowSize:endItrY
        ...
        if endX == size(result,1)
            edge_ex = 1;            
        end
        
        if endY == size(result,2)
            edge_ey = 1;            
        end
        ...
        fprintf('stX = %d, stY = %d, endX = %d, endY = %d\n',stX,stY,endX,endY);
        [seg,myMask] = compute_nuclei_per_patch_cnn_bigSize_v2(stX,stY,endX,endY,result,double_scale,scale,resizeScale,diskScale,tempPath);
        ...
        % haye akse original
        [~,curSegBoundaries] = find_boundary_of_each_nuclei_v2(seg,sstX-1,sstY-1,inputSize,edge_sx,edge_sy,edge_ex,edge_ey);
